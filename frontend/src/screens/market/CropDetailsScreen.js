...
import { View, Text, StyleSheet, ScrollView, TouchableOpacity, TextInput, Alert, ActivityIndicator, Image, Dimensions, FlatList } from 'react-native';
        ...
        <View style={styles.container}>
            ...
            <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
                ...
                {crop.images && crop.images.length > 0 && (
                    <View style={styles.imageGallerySection}>
                        <View style={styles.galleryHeader}>
                            ...
                            <Text style={styles.gallerySectionTitle}>{t('crop_images') || 'Crop Images'}</Text>
                            <Text style={styles.imageCounter}>{activeImageIndex + 1}/{crop.images.length}</Text>
                        </View>
                        <FlatList
                            data={crop.images}
                            horizontal
                            ...
                            onMomentumScrollEnd={(e) => {
                                const slideWidth = screenWidth - 72;
                                const index = Math.round(e.nativeEvent.contentOffset.x / slideWidth);
                                setActiveImageIndex(Math.max(0, Math.min(index, crop.images.length - 1)));
                            }}
                ...
                <View style={styles.section}>
                    <Text style={styles.priceTag}>{t('current_price') || 'Current Price'}: ₹{Number(crop.current_price).toLocaleString('en-IN')}/{crop.unit}</Text>
                ...
                {user?.user_type === 'buyer' && crop.status === 'listed' && (
                    <View style={styles.bidSection}>
                        <View style={styles.bidHeader}>
                            ...
                            <Tooltip text={t('place_bid_tooltip') || 'Enter your bid amount to make an offer. Your bid must be at least the minimum price. The farmer will review and respond to your bid.'} iconSize={16} />
                        </View>
                        <TextInput
                            style={styles.input}
